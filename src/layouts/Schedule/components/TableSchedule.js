    ...
    const columns = [
        {
            ...
            render: (data) => (
                <div className="user__content" >
                    <Link to={`${path}/edit/${data.id}`} style={{ color: "#52889e" }} >{data.target_type === "3" ? "Leads" : data.receiver}</Link>
        ...
        },
        {
            ...
            render: (data) => (
                <div>
                    ...
                    {data.archived == 1 ? <button className="btn btn-outline-warning  btn-sm h-auto lh-10 p-1 mb-2 mb-lg-0 mr-0 "
                        style={{ borderRadius: "50%" }}
                        onClick={() => App.deleteModal(() => onActive(data.id))} >

                        <i className="feather-rotate-ccw" />
                    </button> :
                        <button className="btn btn-outline-primary  btn-sm h-auto lh-10 p-1 mb-2 mb-lg-0 mr-0 "
                            style={{ borderRadius: "50%" }}
                    ...
                    <button className="btn btn-outline-danger 
                       btn btn-outline-danger btn-sm h-auto lh-10 p-1 mb-2 mb-lg-0 mr-0 mx-2"
                        style={{ borderRadius: "50%" }}
                        onClick={() => App.deleteModal(() => onDelete(data.id))}><i className="feather feather-x"></i></button>
