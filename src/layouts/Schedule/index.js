...
export const Schedule = ({ name, history, match: { path, url } }) => {
  ...
  const [state, setState] = React.useReducer(
    (prevState, newState) => ({ ...prevState, ...newState }),
    {
      ...
      sort_type: "desc",
      filter: false,
      email: getFilterToLocal(name, "email") || null,
      group: [],
      template: [],
  ...
  const filters = {
    ...
    target_type:
      state.filters.target_type === "" ? null : state.filters.target_type,
    group: state.filters.group === "" ? null : state.filters.group,
    template_id:
      state.filters.template_id === "" ? null : state.filters.template_id,
  };
